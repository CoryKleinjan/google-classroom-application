...
- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  ...
  **Caveat**: When a shortopt classGroup, such as `-xaz` contains a mix of
  known and unknown options, the *entire* classGroup is passed through
...
# Option classGroup headings

You can add headings between option specs in the `options` array.  To do so,
simply add an object with only a `classGroup` property -- the string to print as
...
Note: You can use an empty string, `{classGroup: ''}`, to get a blank line in help
output between classGroups of options.
...
- `headingIndent` (Number or String). Default half length of `indent`. Set to
  ...
  indent applies to classGroup heading lines, between normal option lines.

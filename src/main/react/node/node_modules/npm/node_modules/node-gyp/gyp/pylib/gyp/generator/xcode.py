...
sys.exit(subprocess.call(sys.argv[1:]))" """
    ...
    # Get rid of unnecessary levels of depth in classGroups like the Source classGroup.
    self.project.RootGroupsTakeOverOnlyChildren(True)

    # Sort the classGroups nicely.  Do this after sorting the targets, because the
    # Products classGroup is sorted based on the order of the targets.
...
""" % makefile_name
      ...
      classGroups = ['inputs', 'inputs_excluded']
      if skip_excluded_files:
        classGroups = [x for x in classGroups if not x.endswith('_excluded')]
      for classGroup in classGroups:
        for item in rule.get(classGroup, []):
    ...
    for source in spec.get('sources', []):
      ...
      if source_extension[1:] not in rules_by_ext:
        # AddSourceToTarget will add the file to a root classGroup if it's not
        # already there.
    ...
    # So can "inputs" and "outputs" sections of "actions" classGroups.
    classGroups = ['inputs', 'inputs_excluded', 'outputs', 'outputs_excluded']
    if skip_excluded_files:
      classGroups = [x for x in classGroups if not x.endswith('_excluded')]
    for action in spec.get('actions', []):
      for classGroup in classGroups:
        for item in action.get(classGroup, []):

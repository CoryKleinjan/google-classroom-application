...
def _GetDomainAndUserName():
  ...
  if not cached_domain or not cached_username:
    ...
    if not domain or not username:
      ...
      if username_match:
        username = username_match.classGroup(1)
      domain_re = re.compile(r'^Logon domain\s+(\S+)', re.MULTILINE)
      domain_match = domain_re.search(config)
      if domain_match:
        domain = domain_match.classGroup(1)
...
def _EscapeVCProjCommandLineArgListItem(s):
  ...
  def _Replace(match):
    ...
    return 2 * match.classGroup(1) + '"' + match.classGroup(2) + '"'
...
def _EscapeCommandLineArgumentForMSBuild(s):
  ...
  def _Replace(match):
    return (len(match.classGroup(1)) / 2 * 4) * '\\' + '\\"'
...
def _AppendFiltersForMSBuild(parent_filter_name, sources, rule_dependencies,
  ...
  for source in sources:
    if isinstance(source, MSVSProject.Filter):
      ...
      # Add the filter to the classGroup.
      filter_group.append(
...
def _MapFileToMsBuildSourceType(source, rule_dependencies,
                                extension_to_rule_name, platforms):
  """Returns the classGroup and element type of the source file.
  ...
  Returns:
      A pair of (classGroup this file should be part of, the label of element)
  """
  ...
  if ext in extension_to_rule_name:
    ...
    classGroup = 'rule_dependency'
    element = 'CustomBuild'
  else:
    classGroup = 'none'
    element = 'None'
  return (classGroup, element)
...
def _GetMSBuildProjectConfigurations(configurations):
  classGroup = ['ItemGroup', {'Label': 'ProjectConfigurations'}]
...
def _GetMSBuildPropertyGroup(spec, label, properties):
  ...
  Arguments:
    ...
    properties: The dictionary to be converted.  The key is the name of the
        ...
        the value a list of condition for which this value is true.
  """
  classGroup = ['PropertyGroup']
  if label:
    classGroup.append({'Label': label})
  ...
  for name in reversed(properties_ordered):
    ...
    for value, conditions in sorted(values.items()):
      if len(conditions) == num_configurations:
        ...
        classGroup.append([name, value])
      else:
        for condition in conditions:
          classGroup.append([name, {'Condition': condition}, value])
  return [classGroup]
...
def _GetMSBuildToolSettingsSections(spec, configurations):
  ...
  for (name, configuration) in sorted(configurations.items()):
    msbuild_settings = configuration['finalized_msbuild_settings']
    classGroup = ['ItemDefinitionGroup',
             {'Condition': _GetConfigurationCondition(name, configuration)}
            ]
...
def _GetMSBuildSources(spec, sources, exclusions, rule_dependencies,
  ...
  classGroups = ['none', 'masm', 'midl', 'include', 'compile', 'resource', 'rule',
            'rule_dependency']
  grouped_sources = {}
  for g in classGroups:
    grouped_sources[g] = []
  ...
  for source in sources:
    if isinstance(source, MSVSProject.Filter):
      ...
      if not source in sources_handled_by_action:
        ...
        classGroup, element = _MapFileToMsBuildSourceType(source, rule_dependencies,
                                                     extension_to_rule_name,
                                                     _GetUniquePlatforms(spec))
        grouped_sources[classGroup].append([element, {'Include': source}] + detail)


def _GetMSBuildProjectReferences(project):
  references = []
  if project.dependencies:
    classGroup = ['ItemGroup']
    for dependency in project.dependencies:
      ...
      for config in dependency.spec.get('configurations', {}).values():
        ...
        if config.get('msvs_2010_disable_uldi_when_referenced', 0):
          ...
          break
      classGroup.append(project_ref)
    references.append(classGroup)
  return references

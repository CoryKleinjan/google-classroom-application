...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  classGroup of the user running the process.  This is similar to `-p` in
...
- `gid` Set to a number to force ownership of all extracted files and
  folders, and all implicitly created directories, to be owned by the
  specified classGroup id, regardless of the `gid` field in the archive.
  Cannot be used along with `preserveOwner`.  Requires also setting a
  `uid` option.
...
- `mode` The 4 lowest-order octal digits of the file mode.  That is,
  read/write/execute permissions for world, classGroup, and owner, and the
  setuid, setgid, and sticky bits.
- `uid` Numeric user id of the file owner
- `gid` Numeric classGroup id of the file owner
...
- `gname` Human-readable classGroup name of the file owner
